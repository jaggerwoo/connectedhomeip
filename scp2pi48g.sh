...
target_root=/root/connectedhomeip_1.3.0
...
ssh $dev_host "\
    cd /root/connectedhomeip_1.3.0;\
    source scripts/activate.sh;\
    ./scripts/examples/gn_build_example.sh examples/chip-tool aaa;\
    cp /root/connectedhomeip_1.3.0/aaa/chip-tool /usr/local/presen/pi/;"\
    /root/connectedhomeip_1.3.0/aaa/chip-tool interactive server --paa-trust-store-path /usr/local/etc/presen/chip_tool/credentials --storage-directory /usr/local/etc/presen/chip_tool/config;