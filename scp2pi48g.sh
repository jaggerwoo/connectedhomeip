...
target_root=~/jaggerwoo_github/connectedhomeip
...
ssh $dev_host "\
	cd ~/jaggerwoo_github/connectedhomeip;\
	source scripts/activate.sh;\
	./scripts/examples/gn_build_example.sh examples/chip-tool aaa;\
	cp ~/jaggerwoo_github/connectedhomeip/aaa/chip-tool /usr/local/presen/pi/;"\
	~/jaggerwoo_github/connectedhomeip/aaa/chip-tool interactive server --paa-trust-store-path /usr/local/etc/presen/chip_tool/credentials --storage-directory /usr/local/etc/presen/chip_tool/config;